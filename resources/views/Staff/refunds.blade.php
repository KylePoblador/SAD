        ...
        <div class="rounded-xl border border-emerald-100 bg-white p-5 shadow-sm">
            ...
            <form id="refund-form" class="space-y-4">
                ...
                <div>
                    ...
                    <div id="student-suggestions" class="mt-2 hidden space-y-1"></div>
                    <input type="hidden" id="student_user_id" name="student_user_id">
                    <span id="selected-student" class="text-sm text-gray-600"></span>
                </div>
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    const response = await fetch(
                        `/student/connect/search?q=${encodeURIComponent(query)}`);
                    const students = await response.json();

                    const suggestions = document.getElementById('student-suggestions');
                    if (students.length > 0) {
                        suggestions.innerHTML = students.map(student => `
                            <div class="cursor-pointer rounded-lg border border-gray-200 p-2 hover:bg-gray-50">
