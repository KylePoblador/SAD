@php
    $topbarCartCount = 0;
    foreach (session('student_carts', []) as $lines) {
        if (is_array($lines)) {
            $topbarCartCount += (int) collect($lines)->sum(fn ($l) => (int) ($l['qty'] ?? 0));
        }
    }
@endphp
<div class="sticky top-0 z-10 border-b border-gray-200 bg-white shadow-sm">
    <div class="coinmeal-container flex items-center justify-between py-3">
        ...
        <div class="flex shrink-0 items-center gap-3 sm:gap-4">
            <a href="{{ route('student.cart.hub') }}" class="relative text-gray-500 hover:text-green-600" aria-label="Cart">
                ...
                @if ($topbarCartCount > 0)
                    <span id="topbar-cart-badge"
                        class="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-green-600 text-[10px] font-bold text-white">{{ $topbarCartCount > 99 ? '99+' : $topbarCartCount }}</span>
                @endif
            </a>
